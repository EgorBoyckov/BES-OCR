...
def extract_color_marks(
    ...
    for i in range(1, n):
        ...
        # Убираем отметку из растра для OCR: цветной пиксель заменяется его
        # самым ярким каналом (синие чернила в синем канале почти белые,
        # красные — в красном), а не белым цветом. Так чёрный текст там, где
        # его пересекают штрихи печати, остаётся тёмным и читается, а сама
        # печать исчезает.
        erase = cv2.dilate(comp_mask, np.ones((3, 3), np.uint8)) > 0
        region = cleaned[y0:y1, x0:x1]
        brightest = region.max(axis=2)
        # Небольшое осветление: бледные края оттиска не должны дать серый
        # "ореол" после бинаризации.
        lifted = np.clip(brightest.astype(np.int16) + 40, 0, 255).astype(np.uint8)
        region[erase] = np.repeat(lifted[erase][:, None], 3, axis=1)
